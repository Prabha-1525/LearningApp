...
import type {ShapeQuizQuestion} from '../../domain/entities/shapeEntities';
import {shapesAudio} from '../../domain/audio/shapesAudioEngine';

interface ShapeQuizEngineProps {
  readonly questions: readonly ShapeQuizQuestion[];
  readonly accentColor?: string;
  readonly onFinish: (score: number, stars: number) => void;
}

export function ShapeQuizEngine({
  questions,
  accentColor = '#EC4899',
  onFinish,
}: ShapeQuizEngineProps) {
  ...
  useEffect(() => {
    if (currentQ?.promptAudio) {
      shapesAudio.speak(currentQ.promptAudio);
    }
  }, [currentIdx, currentQ]);
  ...
  const handleSelectOption = (opt: (typeof currentQ.options)[number]) => {
    ...
    if (isCorrect) {
      ...
      shapesAudio.speak(`Awesome! ${currentQ.explanation}`);
      ...
      shapesAudio.speak(`Nice try! ${currentQ.explanation}`);
    }
  };
